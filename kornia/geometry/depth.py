...
def depth_to_3d(depth: Tensor, camera_matrix: Tensor, normalize_points: bool = False) -> Tensor:
    ...
    # Efficiently get/copy meshgrid to needed device/dtype only once.
    points_2d = create_meshgrid(H, W, normalized_coordinates=False, device=depth.device, dtype=depth.dtype)
    # Since points_2d is shape (1, H, W, 2), broadcasting works for B batches below.
    points_depth = depth.permute(0, 2, 3, 1)  # BxHxWx1

    camera_matrix_exp = camera_matrix[:, None, None]  # Bx1x1x3x3
    points_3d = unproject_points(points_2d, points_depth, camera_matrix_exp, normalize=normalize_points)  # BxHxWx3
