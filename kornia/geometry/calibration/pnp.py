...
def _mean_isotropic_scale_normalize(points: torch.Tensor, eps: float = 1e-8) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    # Use keepdim for scale calculation to reduce temp allocations
    scale = (points - x_mean).norm(dim=-1, p=2)
    scale = scale.mean(dim=1)  # B

    D_int = points.shape[-1]
    D_float = float(D_int)
    scale = torch.sqrt(torch.tensor(D_float, dtype=points.dtype, device=points.device)) / (scale + eps)  # B

    B = points.shape[0]
    # Create the transformation matrix efficiently (reuse created identity and use broadcasting)
    transform = torch.eye(D_int + 1, dtype=points.dtype, device=points.device).expand(B, D_int + 1, D_int + 1).clone()
    idxs = torch.arange(D_int, dtype=torch.long, device=points.device)
    # Transform diagonals with broadcasting
