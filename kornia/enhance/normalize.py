...
def normalize_min_max(x: Tensor, min_val: float = 0.0, max_val: float = 1.0, eps: float = 1e-6) -> Tensor:
    ...
    # Instead of repeated .view(), use keepdim=True for min/max
    B, C = x.shape[:2]
    ...
    # The broadcasting works thanks to keepdim=True above; no .view() needed
    scale = max_val - min_val
    x_out = scale * (x - x_min) / (x_max - x_min + eps) + min_val
